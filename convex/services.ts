...
export const getBusinessServices = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Get services offered by this business
    const serviceNames = businessProfile.servicesOffered || []
    ...
    const services = await Promise.all(
      serviceNames.map(async (serviceName) => {
        const service = await ctx.db
          .query("servicePackages")
          .filter((q) => q.eq(q.field("name"), serviceName))
          .first()
        return service
      }),
    )
...
export const addServiceToBusiness = mutation({
  args: {
    businessId: v.id("businessProfiles"),
  ...
  handler: async (ctx, args) => {
    ...
    const currentServices = businessProfile.servicesOffered || []
    if (!currentServices.includes(args.serviceName)) {
      await ctx.db.patch(args.businessId, {
        servicesOffered: [...currentServices, args.serviceName],
        updatedAt: new Date().toISOString(),
      })
    }
  },
...
// Remove a service from a business
export const removeServiceFromBusiness = mutation({
  args: {
    businessId: v.id("businessProfiles"),
    serviceName: v.string(),
  },
  handler: async (ctx, args) => {
    ...
    const currentServices = businessProfile.servicesOffered || []
    const updatedServices = currentServices.filter((service) => service !== args.serviceName)
