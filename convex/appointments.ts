...
export const getBusinessAppointments = query({
  ...
  handler: async (ctx, args) => {
    ...
    const appointments = await appointmentsQuery.take(args.limit || 10)
...
// Add business ID to appointments for efficient querying
export const addBusinessIdToAppointments = mutation({
  args: {
    appointmentId: v.id("appointments"),
    businessId: v.id("businessProfiles"),
  },
